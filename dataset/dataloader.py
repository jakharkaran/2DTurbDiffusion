...
class CustomMatDatasetEmulator(Dataset):
    def __init__(self, data_dir, file_range, downsample_factor=1, downsample_procedure='physical', get_UV=True, get_Psi=False, get_Omega=False, normalize=True):
        """
        Args:
            data_dir (str): Directory with all the .mat files.
        ...
        self.file_batch_size = 50 # Each file contains 50 samples
        self.file_range = file_range
        filenum1 = file_range[0] // self.file_batch_size + file_range[0] % self.file_batch_size
        filenum2 = file_range[1] // self.file_batch_size + file_range[1] % self.file_batch_size
        print('** filenum **', filenum1, filenum2)

        self.data_dir = data_dir
        self.file_numbers = range(filenum1, filenum2 + 1)
        self.file_list = [os.path.join(data_dir, 'data', f"{i}.npy") for i in self.file_numbers]
        self.downsample_factor = downsample_factor
        self.downsample_procedure = downsample_procedure
        self.get_UV = get_UV
        self.get_Psi = get_Psi
    ...
    def __len__(self):
        return self.file_range[1] - self.file_range[0] + 1
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            torch.Tensor: Data loaded from the .npy file.
        """
        if torch.is_tensor(idx):
            idx = idx.tolist()

        batch_ind = idx//self.file_batch_size
        file_path = self.file_list[batch_ind]
        mat_data = np.load(file_path)

        file_num = idx % self.file_batch_size
        U = mat_data[file_num,0,:,:]
        V = mat_data[file_num,1,:,:]

        # print(f'idx:{idx} file_num:{file_num} batch_ind:{batch_ind} batch_size:{self.file_batch_size} file_path:{file_path}')
        ...
        if self.get_Psi or self.get_Omega:
